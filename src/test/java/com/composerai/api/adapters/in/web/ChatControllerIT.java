...
import com.composerai.api.application.usecase.chat.StreamChatUseCase;
import com.composerai.api.config.AiFunctionCatalogProperties;
import com.composerai.api.controller.SystemController;
...
@Import(ChatControllerIT.TestConfig.class)
class ChatControllerIT {
    ...
    void chatEndpoint_WithEmailContextOnly_ShouldSucceed() throws Exception {
        ...
        Mockito.when(executeChatUseCase.execute(any())).thenReturn(chatResponse);
    ...
    void chatEndpointAcceptsEveryCanonicalReasoningEffort(ReasoningEffortLevel reasoningEffort) throws Exception {
        ChatResponse chatResponse = new ChatResponse("Hi", "conv-1", java.util.List.of(), "answer", "<p>Hi</p>");
        Mockito.when(executeChatUseCase.execute(any())).thenReturn(chatResponse);

        ChatRequest request = new ChatRequest("Hi", null, 5);
        request.setThinkingEnabled(true);
        request.setThinkingLevel(reasoningEffort);
    ...
    @Test
    void chatEndpointRejectsInvalidNonblankReasoningEffort() throws Exception {
        mockMvc.perform(post(CHAT_ENDPOINT)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                            {"message":"Hi","maxResults":5,"thinkingLevel":"turbo"}
                            """))
                .andExpect(status().isBadRequest())
                .andExpect(content().string(containsString("malformed_request")));
    ...
    void streamEndpoint_ShouldEmitSanitizedHtmlChunks() throws Exception {
        ...
        doAnswer(invocation -> {
                    ...
                    callbacks.onHtmlChunk().accept("<p>Hello <strong>World</strong></p>");
                    callbacks.onReasoning().accept(null);
        ...
        Mockito.verify(streamChatUseCase)
                .execute(
                        any(ChatRequest.class),
                        org.mockito.ArgumentMatchers.anyString(),
                        org.mockito.ArgumentMatchers.anyString(),
