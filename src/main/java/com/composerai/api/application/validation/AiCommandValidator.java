package com.composerai.api.application.validation;
...
public class AiCommandValidator implements ConstraintValidator<AiCommandValid, ChatRequest> {
    ...
    @Override
    public boolean isValid(ChatRequest request, ConstraintValidatorContext context) {
        if (request == null) {
            return true;
        }
        ...
        Optional<AiFunctionDefinition> definition = catalogHelper.find(command, request.getCommandVariant());
        if (definition.isEmpty()) {
            reject(context, "aiCommand", "Unknown aiCommand or variant");
            return false;
        }

        if (definition.get().requiresSubject() && StringUtils.isBlank(request.getSubject())) {
