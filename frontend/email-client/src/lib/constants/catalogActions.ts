 ...
 */
export type MailboxActionFallback = {
  key: string;
  label: string;
  description: string;
  comingSoon?: boolean;
...
] as const satisfies readonly MailboxActionFallback[];
...
] as const satisfies readonly ActionMenuOption[];

/**
 * Prioritization order for toolbar catalog commands so high-value actions surface first.
 */
export type ToolbarCommandPreference = 'draft' | 'translate';

export const PRIMARY_TOOLBAR_PREFERENCE = ['draft', 'translate'] as const satisfies readonly ToolbarCommandPreference[];

/**
 * Catalog command invoked by the “Action ideas” panel.
 */
export const ACTION_MENU_COMMAND_KEY = 'actions_menu' as const;
