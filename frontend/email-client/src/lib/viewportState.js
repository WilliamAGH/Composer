...
function computeMatches(measurement, queries) {
  if (queries) {
    return {
...
function registerMediaQueryListener(mql, handler) {
  if (!mql) return () => {};
 ...
 */
export const viewportState = readable(DEFAULT_STATE, (set) => {
  if (!isBrowser) return () => {};
